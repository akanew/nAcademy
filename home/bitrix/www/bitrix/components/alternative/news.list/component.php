...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			foreach($sections_id as $section){
				$arFilter = Array("IBLOCK_ID"=>$arItem["IBLOCK_ID"], "SECTION_ID" => $section["ID"]);
				$res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter, false, false, Array("ID", "NAME"));
				while($ar_fields = $res->GetNext())
					if($ar_fields["ID"] == $arItem["ID"])
						$arItem["SECTION_INFO"] = $section;
			}
